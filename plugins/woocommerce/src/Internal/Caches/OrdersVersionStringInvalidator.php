...
class OrdersVersionStringInvalidator {
	...
	private function register_hooks(): void {
		// WordPress post hooks for orders (only when HPOS is disabled).
		if ( ! OrderUtil::custom_orders_table_usage_is_enabled() ) {
			add_action( 'save_post_shop_order', array( $this, 'handle_save_post_shop_order' ), 10, 1 );
		...
		// WooCommerce CRUD hooks for orders (work for both HPOS and legacy).
		...
		// Status change hook (fires for both HPOS and legacy, includes from/to statuses).
		...
		// Refund CRUD hooks.
		add_action( 'woocommerce_new_order_refund', array( $this, 'handle_woocommerce_new_order_refund' ), 10, 2 );
		add_action( 'woocommerce_update_order_refund', array( $this, 'handle_woocommerce_update_order_refund' ), 10, 2 );
	}
	...
	public function handle_before_order_save( $order ): void {
		...
		if ( $order_id > 0 ) {
			// Store the current customer ID before changes are applied.
			$this->pre_save_customer_ids[ $order_id ] = (int) $order->get_data()['customer_id'];
		}
	}

	/**
	 * Handle the save_post_shop_order hook.
	 *
	 ...
	 * @internal
	 */
	public function handle_save_post_shop_order( $post_id ): void {
		$post_id = (int) $post_id;
	...
	public function handle_delete_post( $post_id, $post = null ): void {
		...
		if ( 'shop_order_refund' === $post->post_type ) {
			...
			$this->invalidate_refund( $post_id );
			$this->invalidate( $parent_id );
			$this->invalidate_order_refunds_list( $parent_id );
	 ...
	 *
	 * Invalidates the orders list version string when an order
	 * changes status, as this may affect which orders appear in collection endpoints.
	 *
	...
	public function handle_transition_post_status( $new_status, $old_status, $post ): void {
		...
		if ( 'shop_order' === $post->post_type ) {
			$this->invalidate_orders_list();
		} elseif ( 'shop_order_refund' === $post->post_type ) {
			$this->invalidate_refunds_list();
			$this->invalidate_order_refunds_list( (int) $post->post_parent );
		}
	}
	...
	public function handle_woocommerce_update_order( $order_id, $order ): void {
		...
		// Check if customer changed - if so, invalidate the orders list.
		if ( $this->did_customer_change( $order_id, $order ) ) {
			$this->invalidate_orders_list();
		}

		// Clean up stored customer ID.
	...
	/**
	 * Handle the woocommerce_new_order_refund hook.
	 *
	 * @param int              $refund_id The refund ID.
	 ...
	 * @internal
	 */
	public function handle_woocommerce_new_order_refund( $refund_id, $refund ): void {
		$refund_id = (int) $refund_id;
		$parent_id = $refund instanceof \WC_Order_Refund ? (int) $refund->get_parent_id() : 0;
	...
	/**
	 * Handle the woocommerce_update_order_refund hook.
	 *
	 * @param int              $refund_id The refund ID.
	 * @param \WC_Order_Refund $refund    The refund object.
	 *
	 ...
	 * @internal
	 */
	public function handle_woocommerce_update_order_refund( $refund_id, $refund ): void {
		$refund_id = (int) $refund_id;
		$parent_id = $refund instanceof \WC_Order_Refund ? (int) $refund->get_parent_id() : 0;
